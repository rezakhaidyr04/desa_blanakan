        ...
        <form action="{{ route('admin.officials.update', $official) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            @if($official->photo)
                <div>
                    ...
                    <img src="{{ asset('storage/' . $official->photo) }}" alt="{{ $official->name }}" class="w-24 h-24 object-cover rounded-full">
                </div>
            @endif

            <div>
                <label for="photo" class="block text-sm font-medium text-slate-700 mb-2">{{ $official->photo ? 'Ganti Foto' : 'Upload Foto' }}</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $official->phone) }}"
                        class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-slate-700 mb-2">Email</label>
                    <input type="email" id="email" name="email" value="{{ old('email', $official->email) }}"
                        class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="bio" name="bio" rows="3"
                    class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">{{ old('bio', $official->bio) }}</textarea>
            </div>

            <div>
                <label for="order" class="block text-sm font-medium text-slate-700 mb-2">Urutan Tampil</label>
                <input type="number" id="order" name="order" value="{{ old('order', $official->order) }}"
                    class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
            </div>

            <div class="flex items-center">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ $official->is_active ? 'checked' : '' }}
