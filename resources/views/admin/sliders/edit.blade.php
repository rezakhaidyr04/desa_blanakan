    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <img src="{{ asset('storage/' . $slider->image) }}" alt="{{ $slider->title }}" class="w-full max-w-md h-40 object-cover rounded-lg mb-2">
            </div>

            <div>
                <label for="image" class="block text-sm font-medium text-slate-700 mb-2">Ganti Gambar</label>
                <input type="file" id="image" name="image" accept="image/*"
                    class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                <p class="mt-1 text-sm text-slate-500">Kosongkan jika tidak ingin mengganti.</p>
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ $slider->is_active ? 'checked' : '' }}
