...
<div class="max-w-4xl">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="p-6 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500">Ubah informasi berita</p>
        </div>
        ...
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <select id="category" name="category" required
                    ...
                    <option value="Pembangunan" {{ old('category', $post->category) == 'Pembangunan' ? 'selected' : '' }}>Pembangunan</option>
                    <option value="Sosial" {{ old('category', $post->category) == 'Sosial' ? 'selected' : '' }}>Sosial</option>
            ...
            <div>
                ...
                <textarea id="excerpt" name="excerpt" rows="3" required
                    ...
                    placeholder="Ringkasan singkat berita (maksimal 500 karakter)">{{ old('excerpt', $post->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="body" name="body" rows="10" required
                    ...
                    placeholder="Tulis isi berita lengkap di sini...">{{ old('body', $post->body) }}</textarea>
                @error('body')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($post->image)
                    <div class="mb-4 flex items-center gap-4">
                        <img src="{{ $post->image }}" alt="{{ $post->title }}" class="w-32 h-24 object-cover rounded-lg">
